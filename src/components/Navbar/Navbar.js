            ...
            <ul className="navbar-nav mx-auto me-auto mb-2 mb-lg-0">
              ...
              <li className="nav-item dropdown">
                ...
                <ul className="dropdown-menu" aria-labelledby="navbarDropdown">
                  <li
                    onClick={() => {
                      setUrl({ url: "", type: false });
                      setCurrentTypes("All Types");
                    }}
                  >
                    <p className="dropdown-item">All Types</p>
                  </li>
                  {types.map((e, index) => {
                    return (
                      <li
                        key={index}
                        className={`poke-${e.name}`}
                        onClick={() => {
                          handleType(e);
                        }}
                      >
                        <p className="dropdown-item">{e.name}</p>
                      </li>
                    );
                  })}
                </ul>
